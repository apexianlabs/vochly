...
const Logo = ({ size = 32 }) => (
  <svg width={size} height={size} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
    <defs><linearGradient id="lg" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor="#fbbf24"/><stop offset="100%" stopColor="#b45309"/></linearGradient></defs>
    <path d="M15 20 Q15 12 23 12 L77 12 Q85 12 85 20 L85 58 Q85 66 77 66 L40 66 L25 80 L28 66 L23 66 Q15 66 15 58 Z" fill="url(#lg)"/>
    <circle cx="36" cy="39" r="7" fill="white" opacity="0.9"/><circle cx="50" cy="39" r="7" fill="white" opacity="0.9"/><circle cx="64" cy="39" r="7" fill="white" opacity="0.9"/>
  </svg>
)


const GOOGLE_AUTH_URL = `https://wdeilbhnsdlmckovicqy.supabase.co/auth/v1/authorize?provider=google&redirect_to=https%3A%2F%2Fapp.vochly.app%2Fauth%2Fcallback&scopes=email+profile`
...
export default function LoginPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const body = tab === 'login'
        ? { email: form.email, password: form.password, product: 'vochly' }
        : { email: form.email, password: form.password, full_name: form.name, source_product: 'vochly' }
      ...
      document.cookie = `voc_token=${token}; path=/; max-age=2592000; SameSite=Lax`
      document.cookie = `voc_user=${encodeURIComponent(JSON.stringify({ id: user.id, email: user.email, name: fullName }))}; path=/; max-age=2592000; SameSite=Lax`
      ...
      if (tab === 'signup') {
        fetch(`${process.env.NEXT_PUBLIC_DB_API_URL}/email/welcome`, {
          ...
          body: JSON.stringify({ product: 'vochly', to: form.email, name: fullName, email: form.email })
  ...
  const leftPanel = (
    <div style={{width:420,flexShrink:0,background:'#d97706',display:'flex',flexDirection:'column',padding:'40px',position:'relative',overflow:'hidden'}}>
      ...
      <Link href="/" style={{display:'flex',alignItems:'center',gap:10,marginBottom:48,zIndex:1,textDecoration:'none'}}>
        <Logo size={36}/>
        <span style={{fontSize:20,fontWeight:800,color:'#fff'}}>Vochly</span>
      </Link>
        ...
        <div style={{width:'100%',maxWidth:400}}>
          {isMobile && (
            <Link href="/" style={{display:'flex',alignItems:'center',gap:8,marginBottom:24,textDecoration:'none',justifyContent:'center'}}>
              <Logo size={36}/>
              <span style={{fontWeight:800,fontSize:18,color:'#0f172a'}}>Vochly</span>
            </Link>
          )}
          ...
          <p style={{fontSize:14,color:'#64748b',marginBottom:20}}>
            {tab === 'login' ? `Sign in to your Vochly account.` : 'Start your free trial today.'}
          </p>
          ...
          <button onClick={handleSubmit} disabled={loading}
            style={{width:'100%',padding:13,borderRadius:10,border:'none',background:'#d97706',color:'#fff',fontSize:15,fontWeight:700,cursor:'pointer',fontFamily:'Inter,sans-serif',marginBottom:16,opacity:loading?0.7:1}}>
